"""
This is a modified version to work with curve fit.

After using it a bit I realized poly was better for my purpose but didn't want to throw this out

note. I'm halfway through some debugging and don't feel like fixing it, so it doesn't work as is.
    ...
    def rec_inner(Z, depth=0):
        ...
        if Z.ndim == 0:
            ...
            if depth > 0:
                ...
                # ret = " + ".join(ret)
                # ret = ret.replace("+ -", "- ")
                stuff = list(zip(["A", "B", "C"], [" = "]*3, ret))
                ret = "\n".join(["".join(s) for s in stuff])
                return ret

    return rec_inner(Z)


def n_hyperparameters(F, F_m, hyper_order, bounds, xspace, n=10):
    ...
    if type(F_m) is int:
        ...
        sig = signature(F_m)
        F_m_order = len(sig.parameters) - 2
        use_curve_fit = True
    ...
    def rec_inner(Z, depth=0):
        ...
        if depth == n_hyper - 1:
            return np.polyfit(boundspaces[depth], Z, hyper_order)
        else:
    ...
    for idx in itertools.product(*[list(range(n)) for _ in range(n_hyper)]):
        # Z_all[idx] = np.polyfit(xspace, F(xspace, *[boundspaces[i][idx[i]] for i in range(n_hyper)]), F_m_order)
        hypers = [boundspaces[i][idx[i]] for i in range(n_hyper)]
        ratios = F(xspace, *hypers)
        if use_curve_fit:
            popt, pcov = curve_fit(F_m, ratios, xspace)#, bounds=((-np.inf, -np.inf, -np.min(ratios)), np.inf))
        zs = np.polyfit(ratios, xspace, F_m_order)
        Z_all[idx] = popt

        debug = False
        _ = 0
        if debug:
            plt.plot(ratios, xspace, 'r*')
            x = np.linspace(ratios[0], ratios[-1], 100)
            plt.plot(x, F_m(x, *popt))
            print(popt)
            plt.show()
            # quit()
