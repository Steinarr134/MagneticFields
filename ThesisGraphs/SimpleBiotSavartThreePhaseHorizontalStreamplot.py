...
# fig.suptitle(r"Streamplot of the $\mathbf{B}$ field around 3 horizontal phases")
axs = np.ravel(axs)
...
for n, t in enumerate(timepoints):
    ...
    lw_offset = 13
    # for p in range(3):
    #     lw = np.log(B_phase_strength[::a, ::a, p]) + lw_offset
    #     print(lw)
    ...
    axs[n].set_xlabel("x location[m]")
    # axs[0].set_ylabel("y location[m]")
    axs[n].set_ylabel("y location[m]")
# fig.savefig(f"Graphs\\SimpleBiotSavartThreePhaseHorizontalStreamplots.pdf", format='pdf', bbox_inches='tight')
plt.show()